...
| `JARVIS_VOICE_NAME` | Optional display name for the selected voice |
| `JARVIS_PUBLIC_HOST` | Hostname only, such as `jarvis.vavqo.com`; no scheme or path |

If ElevenLabs variables are omitted, the HUD can still load and typed chat can
work, but spoken audio will report that ElevenLabs is not configured.
